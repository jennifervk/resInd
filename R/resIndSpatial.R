...
#'
#'\dontrun{
#'#Run function "resIndSpatial". This requires the function 'mc.calc' from the package "bfastSpatial", which you can install from github \url{https://github.com/loicdtx/bfastSpatial} .
...
resIndSpatial <- function(x, dates, type='irregular', sc=1, order=3,
  ...
  fun <- function(x){
    ...
    ##Set own NoData value to differentiate between "no data pixels" and "no result/no breakpoint pixels"
    NV=NV
    ...
    if(!all(is.na(bfts))){
      ...
      #Add trend prediction for each segment. Corrected hight of trend line for irregular data: sets harmonic term based on mean DOY of observations / segment (instead of trend$harmon[] <- 0, which assumes regular data); idea based on suggestion by Achim Zeileis
...
# Calculate (drought) resilience indicators -------------------------------
      ...
      ##If no (significant) breakpoint fuond set breakpoint position, breakpoint timing, and recovery trend to "NV"; set DBP to 0
      if(bpnumb==0){
        DBP <- 0
        bpt <- NV
        tlag <- NV
        ...
        ##If drought breakpoint occured set DBP to 1 and calculate breakpoint timing, trend of recovery, trend in segment before BP, preNDVI, Magnitude of change (MagA & MagR) based on mean NDVI of s years before and after BP. If there is more than 1 BP in the specified time interval, the first one is chosen!
        if (length(bpd) > 1) {
          bpd <- bpd[1]
        }
...
# Save and return output --------------------------------------------------
  ...
  names(out) <- c('BPNumb', 'Initial NDVI', 'Intercept', 'DBP','BpTime', 'Timelag', 'RecTrend', 'PreTrend', 'PreNDVI', 'MagObsA', 'MagObsR',
